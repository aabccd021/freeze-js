...
for (const script of Array.from(scripts)) {
  ...
  for (const [key, fnName] of Object.entries(script.dataset)) {
    ...
    const hookEntryPromise = import(script.src).then((module) => [exportFnName, module[fnName]]);
...
export const hooks = hooksEntrySettled.filter((result) => result.status === "fulfilled").map((result) => result.value);
